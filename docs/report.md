...
隨著網路文學產值的逐年增長，小說市場變得極為龐大，然而現有依賴手動勾選標籤的結構化檢索系統過於僵硬，無法滿足使用者口語化的搜尋需求。另一方面，直接導入大型語言模型（LLM）的搜尋系統則常出現「語義偏移」與「標籤幻覺」，導致搜尋失效或不穩定。為解決此痛點，本專題開發了一套結合 LLM 與規則導向（Rule-based）邏輯的混合式網路小說檢索引擎，透過「語意映射模組」與「結構化過濾邏輯」，輔以「引導式任務解構」技術，將使用者的模糊意圖精確且穩定地對齊至系統預定義的標籤集。實驗成果顯示，本系統能顯著提升非嚴格查詢的語意檢索能力，同時確保結構化條件的穩定攔截，提升了系統的檢索穩定性與語意對齊精度。
...
![image](https://hackmd.io/_uploads/rJ8XypxJzx.png)
...
### 4.4 實驗四：PermSC Reranking 權重與候選池的動態權衡

#### 4.4.1 實驗設計與背景

在 Hybrid 架構確立後，為了進一步提升最終排序的精準度，本系統引入了基於 LLM 的 PermSC Reranker 進行二次重排。本實驗旨在探討不同「權重比例（標籤屬性 : 語意）」與「候選池大小（Recall Limit）」對最終檢索品質的影響，尋找最佳的 Rerank 配置。
...
### 4.5 實驗五：綜合性能評估與基準對比 (Comprehensive Evaluation)

經過前四項實驗的模組優化與參數調校，本專題之 **Hybrid 引擎** 已達最佳配置。本節將把該引擎置於更宏觀的維度，分別從「規模擴展性（Scalability）」與「檢索品質（Retrieval Quality）」兩個面向，與當前業界常見的兩種極端方案（純 LLM 方案、傳統檢索基準）進行最終對比，以驗證本系統在真實業務場景下的全面優越性。

#### 4.5.1 擴展性與嚴格約束：Hybrid 引擎 vs. 純 LLM 全量掃描

**(1) 實驗背景：挑戰「上下文之牆」 (The Context Wall)**
...
#### 4.5.2 語意理解與綜合品質：Hybrid 引擎 vs. 傳統檢索基準

相較於純 LLM 難以處理極大規模且缺乏穩定性的問題，傳統檢索基準（如 Lexical 關鍵詞或 Naive Hybrid）雖不會超時也無資料量限制，但往往無法深入理解用戶真正的語意意圖。下圖展示了採用不同固定權重的暴力混合基線（Naive Hybrid，灰線）從純向量（0:1）到純關鍵詞（1:0）的表現對比，並以本系統最終強化的 Hybrid 引擎（藍線）作為效能標竿。

```mermaid
%%{init: { 'themeVariables': { 'xyChart': { 'plotColorPalette': '#95A5A6, #95A5A6, #3498DB'} } } }%%
xychart-beta
    title "Avg@10 性能趨勢對比 (No-Strict)"
    x-axis ["0:1", "1:9", "2:8", "3:7", "4:6", "5:5", "6:4", "7:3", "8:2", "9:1", "1:0"]
    y-axis "Avg@10 Score" 0 --> 3
    line [1.90, 1.91, 1.88, 1.78, 1.74, 1.67, 1.49, 1.32, 1.25, 1.20, 1.15]
    line [1.6833, 1.6833, 1.6833, 1.6833, 1.6833, 1.6833, 1.6833, 1.6833, 1.6833, 1.6833, 1.6833]
    line [2.77, 2.77, 2.77, 2.77, 2.77, 2.77, 2.77, 2.77, 2.77, 2.77, 2.77]
```

```mermaid
%%{init: { 'themeVariables': { 'xyChart': { 'plotColorPalette': '#95A5A6, #95A5A6, #3498DB'} } } }%%
xychart-beta
    title "Avg@10 性能趨勢對比 (Strict-Only)"
    x-axis ["0:1", "1:9", "2:8", "3:7", "4:6", "5:5", "6:4", "7:3", "8:2", "9:1", "1:0"]
    y-axis "Avg@10 Score" 0 --> 3
    line [1.01, 1.02, 1.05, 1.03, 1.04, 1.08, 0.97, 0.86, 0.79, 0.77, 0.76]
    line [1.06, 1.06, 1.06, 1.06, 1.06, 1.06, 1.06, 1.06, 1.06, 1.06, 1.06]
    line [2.09, 2.09, 2.09, 2.09, 2.09, 2.09, 2.09, 2.09, 2.09, 2.09, 2.09]
```
*(藍線：Hybrid 引擎（搭配 PermSC Reranking）；灰線：傳統檢索基準)*

實驗數據顯示，即便是傳統方法中表現最好的 `w0.1_v0.9` 比例，其 `no-strict` 最佳分數也僅達 1.9125，`strict-only` 最高也僅達 1.0792。相比之下，本系統最終強化的 **Hybrid 引擎（搭配 PermSC Reranking 6:4）** 在 `no-strict` 分數衝上 **2.7708**（性能提升約 45%），若採用 8:2 權重則在 `strict-only` 亦可達到 **2.0898**（性能提升約 97%）。

這證明本專題的混合系統恰好站在這兩大流派的黃金交集上——既擁有超越傳統檢索的「極高 LLM 語意理解力」，又保有打敗純 LLM「絕對穩定與可擴展」的檢索架構，全面解決了自然語言搜尋長尾與模糊痛點。

### 結論

綜合各項開發測試與實驗結果，本專題得出以下結論：

1. **混合式與任務解構架構大幅提升穩定性**：透過引導式任務解構、槽位填充（Slot filling）與程式端組合（Deterministic merge）技術，本系統能有效解決純 AI 生成篩選條件不穩定的問題。
2. **標籤 (Tag) 主軸與非嚴格檢索的卓越表現**：在保留標籤優先 (Tag-first) 檢索核心特色的前提下，任務解構架構顯著提升了非嚴格查詢的語意檢索能力。
...
雖然引導式任務解構在綜合平均效能與系統可控性上表現優越，但在實驗數據中也發現，單體式聯合建模於最極端的精準排序（Strict-only Strong%）上仍略佔上風。未來研究可探討如何在任務解構架構的高穩定度基礎上，進一步引進晚期融合（Late Fusion）或加權重排序（Reranking，例如引進 Cross-Encoder 模型進行二次重排序）技術，彌補解構式架構在極端精準比對下的微小差距，提供更無懈可擊的 AI 檢索體驗。
