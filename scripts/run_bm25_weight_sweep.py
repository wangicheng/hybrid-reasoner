...
def main() -> None:
    ...
    batch_name = datetime.now().strftime("batch_%Y%m%d_%H%M%S_weightcmp")
    ...
    summary_json = output_dir / "final_decision_table.json"
    summary_json.write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")

    print(f"\n[WeightSweep] Completed. Output directory: {output_dir.as_posix()}")
    print(f"[WeightSweep] Decision table CSV: {decision_csv.as_posix()}")
    print(f"[WeightSweep] Decision table JSON: {summary_json.as_posix()}")
