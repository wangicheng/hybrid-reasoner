...
class ListwiseReranker:
    ...
    async def rerank_candidates(self, query: str, candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        # 打亂順序以規避 LLM 的位置偏差 (Position Bias)
        shuffled_cands = list(candidates)
        random.shuffle(shuffled_cands)
        
        # 建立候選清單文字
        candidates_text = ""
        for i, c in enumerate(shuffled_cands):
...
Below is a list of {len(shuffled_cands)} candidate novels. Each novel has an ID [number], Title, Tags, and a short Intro.
...
"""
        ...
        # 根據 LLM 的排序結果重排，若未選到則排在後面並保持原本順序
        ranked_results = []
        selected_set = set()
        
        # 取出 LLM 選出的 Top 10
        for idx in top_ids:
            if isinstance(idx, int) and 0 <= idx < len(shuffled_cands):
                ranked_results.append(shuffled_cands[idx])
                selected_set.add(shuffled_cands[idx]["book_id"])
                
        # 將剩下的補齊
        for c in candidates:
            if c["book_id"] not in selected_set:
    ...
    try:
        for qi, qc in enumerate(queries, 1):
            ...
            try:
                ...
                print(f"  [Reranker] Scoring {len(candidates)} candidates with Single-Prompt List-wise {MODEL_ID_RERANKER}...")
...
def generate_report():
    ...
    L = []
    L.append("# Gemma 4 List-wise Reranking 實驗報告\n")
    L.append("## 1. 實驗總結\n")
    L.append("在現有的雙軌召回架構（語意向量 + 標籤映射）基礎上，先召回 100 本書，再使用 `gemma-4-31b-it` 對 100 本書進行 **一次性 List-wise 重新排序**。\n")
    L.append("為規避 LLM 在長文本中的「位置偏差 (Position Bias)」，我們採用了 **隨機打亂候選序列 (Shuffle)** 技術，然後讓模型選出 Top 10 並排序。\n")
