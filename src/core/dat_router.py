...
你的任務是評估兩篇候選書籍簡介（文件 A 與 文件 B）對使用者搜尋意圖的「有效性」。

讀者的搜尋意圖通常分為兩種：
1. 精確搜尋：找特定的書名、主角名或專有名詞。
2. 語意搜尋：找特定的劇情套路、風格、流派或世界觀設定（例如：賽博龐克、後宮、系統、高智商）。

請仔細閱讀使用者的搜尋詞與兩本書的簡介，並嚴格按照以下 0~5 分的標準進行獨立評分：
...
JSON 格式必須完全一致如下：
{"plot_score": <文件A的0-5評分>, "bm25_score": <文件B的0-5評分>}
"""
...
_DAT_RESPONSE_SCHEMA = {
    ...
    "required": ["plot_score", "bm25_score"],
}
...
_alpha_cache: Dict[str, Tuple[float, float]] = {}   # {query_hash: (alpha, timestamp)}
...
class DATRouter:
    ...
    def get_dynamic_alpha(
        ...
        """Compute the dynamic α for a given query.
        ...
        info : dict
            Metadata dict with keys: source, plot_score, bm25_score,
            latency_ms, plot_top1_id, bm25_top1_id.
        """
        info: Dict[str, Any] = {
            "source": "fallback",
            "plot_score": None,
        ...
        if cached is not None:
            cached_alpha, cached_ts = cached
            if (time.time() - cached_ts) < CACHE_TTL_SECONDS:
                info["source"] = "cache"
                print(f"[DAT:Cache] Hit for query hash={cache_key[:8]}, α={cached_alpha:.3f}")
                return cached_alpha, info
        ...
        if plot_id and bm25_id and plot_id == bm25_id:
            alpha = self.default_alpha
            info["source"] = "early_exit_same_book"
            print(f"[DAT:EarlyExit] Same book (id={plot_id}), α={alpha:.3f}")
            return alpha, info
        ...
        print(
            f"[DAT:Scout] Plot Top-1: 《{plot_title}》(score={plot_score_raw:.4f}), "
            f"BM25 Top-1: 《{bm25_title}》(score={bm25_score_raw:.2f})"
        )

        # ── LLM Call ──
        started = time.perf_counter()
        try:
            e_plot, e_bm25 = self._llm_score(
                query, plot_title, plot_summary, bm25_title, bm25_summary,
                model_id=model_id,
                plot_top1_raw=plot_top1, bm25_top1_raw=bm25_top1,
            )
            ...
            if e_plot == 0 and e_bm25 == 0:
                ...
                info["source"] = "llm"
                print(
                    f"[DAT:LLM] plot={e_plot}, bm25={e_bm25} → "
                    f"raw_α={raw_alpha:.3f} → clamped_α={alpha:.3f} ({latency_ms:.0f}ms)"
                )
            ...
            print(f"[DAT:Error] LLM failed ({latency_ms:.0f}ms): {exc}, fallback α={alpha:.3f}")

        # ── Cache Result ──
        _alpha_cache[cache_key] = (alpha, time.time())
    ...
    def _llm_score(
        ...
        """Call the LLM to score both candidates.  Returns (plot_score, bm25_score)."""
